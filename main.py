...
def make_pair(data, is_eval=False):  ####
    ...
    for p in items:
        if not is_eval:
            temp_pairs.append(
                (p["tokens"], from_infix_to_prefix(p["expression"])[:MAX_OUTPUT_LENGTH], p["nums"], p["num_pos"]))
        else:
...
def initial_model(output_lang, embedding_size, hidden_size, args, copy_nums, generate_nums):
    ...
    exp_encoder = EncoderGCN(input_size=output_lang.n_words, embedding_size=embedding_size, nhid=hidden_size, hidden_size=hidden_size)
    ...
    if args.model_reload_path != '' and os.path.exists(args.model_reload_path):
        ...
        exp_encoder.load_state_dict(torch.load(os.path.join(args.model_reload_path, "exp_encoder.ckpt")))
        ...
        if args.finetune_from_trainset != "":  # alignment finetune output vocab
            ...
            from_train_data = json.load(
                open(os.path.join(args.data_dir, args.finetune_from_trainset), 'r', encoding='utf-8'))
            from_pairs_trained, from_generate_nums, from_copy_nums = make_pair(from_train_data)
            use_bert = True
            _, from_output_lang, _, _, _ = prepare_data(from_pairs_trained, (), 5, from_generate_nums, from_copy_nums,
                                                        tree=True, use_bert=use_bert, auto_transformer=False,
                                                        bert_pretrain_path=args.bert_pretrain_path)
            op_weight = None
            op_bias = None
            ...
            for i in range(output_lang.num_start):  # op
                ...
                if op_weight == None:
                    ...
                    gene_embed_weight = torch.cat(
                        [gene_embed_weight, gene["embeddings.weight"][from_idx:from_idx + 1, :]], dim=0)
    ...
    return encoder, exp_encoder, predict, generate, merge


def train_model(args, train_pairs, test_pairs, generate_num_ids,
                encoder, exp_encoder, predict, generate, merge, output_lang):
    batch_size = args.batch_size
    need_optimized_parameters = []
    for module in [encoder, exp_encoder, predict, generate, merge]:
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch <= 19:
            ...
            epsilon = math.pow(0.99, epoch - 19)

        temp = 1.0

        input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, exp_num_pos_batches, tarexp_num_lengths, exp_num_index_batches, numslist_batches, tar_adj_batches = prepare_train_batch_infill(
        ...
        for idx in range(len(input_lengths)):
            ...
            loss, loss_math, loss_infill = train_tree(
                input_batches[idx], input_lengths[idx], output_batches[idx], output_lengths[idx],
                num_stack_batches[idx], num_size_batches[idx], generate_num_ids, encoder, exp_encoder, predict, generate, merge,
                output_lang, num_pos_batches[idx], exp_num_pos_batches[idx], tarexp_num_lengths[idx], exp_num_index_batches[idx], numslist_batches[idx], tar_adj_batches[idx], alpha, beam_size, epsilon, temp, hard)
            ...
            encoder.zero_grad()
            exp_encoder.zero_grad()
            predict.zero_grad()
            generate.zero_grad()
            merge.zero_grad()

            loss_total += loss
            loss_math_total += loss_math
            loss_infill_total += loss_infill
            run_steps += 1

            if run_steps % args.logging_steps == 0:
                logger.info(
                    "step: {}, lr: {}, loss_math: {}".format(run_steps, scheduler.get_last_lr()[0], loss_math_total / (idx + 1)))

        logger.info(
            "loss: {}, training time: {}".format(loss_math_total / len(input_lengths), time_since(time.time() - start)))
        logger.info("--------------------------------")
        ...
        if epoch % args.n_val == 0 or epoch > args.n_epochs - 5:
            ...
            for nd in range(len(test_pairs)):
                ...
                logger.info("test_answer_acc: {}, {}".format(float(equation_ac_0) / eval_total,
                                                             float(value_ac_0) / eval_total_0))
            ...
            if float(value_ac) / eval_total > min(best_value_acc_ls):
                ...
                torch.save(encoder.state_dict(),
                           os.path.join(args.output_dir, "epoch_{}".format(epoch), "encoder.ckpt"))
                torch.save(exp_encoder.state_dict(),
                           os.path.join(args.output_dir, "epoch_{}".format(epoch), "exp_encoder.ckpt"))
                torch.save(predict.state_dict(),
                           os.path.join(args.output_dir, "epoch_{}".format(epoch), "predict.ckpt"))
                torch.save(generate.state_dict(),
                           os.path.join(args.output_dir, "epoch_{}".format(epoch), "generate.ckpt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='./models', type=str,
                        help='Model Saved Path, Output Directory')
    parser.add_argument('--bert_pretrain_path', default='./pretrained_models/bert-base-chinese', type=str)
    parser.add_argument('--train_file', default='train.json', type=str)

    parser.add_argument('--finetune_from_trainset', default='', type=str,
                        help='train_file which pretrained model used, important for alignment output vocab')
    ...
    parser.add_argument('--warmup_steps', default=3500, type=int)

    parser.add_argument('--n_save_ckpt', default=3, type=int, help='totally save $n_save_ckpt best ckpts')
    ...
    parser.add_argument('--seed', default=17, type=int, help='universal seed')
    ...
    use_bert = True
    input_lang, output_lang, train_pairs, (test_pairs1, val_pairs1), len_bert_token = prepare_data_infill(pairs_trained, (
    pairs_tested1, pairs_valed1), 5, generate_nums,
                                                                                                   copy_nums, tree=True,
                                                                                                   use_bert=use_bert,
    ...
    encoder, exp_encoder, predict, generate, merge = initial_model(output_lang, embedding_size, hidden_size, args, copy_nums,
                                                      generate_nums)
    if torch.cuda.is_available():
        encoder.cuda()
        exp_encoder.cuda()
