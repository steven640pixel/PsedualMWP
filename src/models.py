...
class EncoderBert(nn.Module):
    def __init__(self, hidden_size, auto_transformer=False, bert_pretrain_path='', dropout=0.5):
        ...
        self.equation_linear = nn.Linear(self.hidden_size, self.hidden_size)
        self.question_linear = nn.Linear(self.hidden_size, self.hidden_size)
        self.elinear = nn.Linear(self.hidden_size, 1)
...
class EncoderGCN(nn.Module):
    def __init__(self, input_size, embedding_size, nhid, hidden_size, dropout=0.5): #input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5
        ...
        self.em_dropout = nn.Dropout(dropout)
        self.dropout = dropout
        self.gc1 = GraphConvolution(embedding_size, nhid)
        self.gc2 = GraphConvolution(nhid, hidden_size)

    def forward(self, input_seqs, adj): #input_seqs, input_lengths, hidden=None
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        embedded = self.em_dropout(embedded)
        embedded = embedded.view(input_seqs.size(1), -1, self.embedding_size)
        x = F.relu(self.gc1(embedded, adj))
        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc2(x, adj)
