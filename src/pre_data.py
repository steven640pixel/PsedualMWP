...
def pad_adj(adj, output_len_max):
    padadj = np.zeros((output_len_max, output_len_max))
...
def PreExpTree(data):
    ...
    while data:
        ...
        if not isOper(tmp):
            ...
            id_p = Et(id)
            id_p.left = index.pop()
            id_p.right = index.pop()

            re.append(p)
            index.append(id_p)

    return re.pop(), index.pop()


def preorder_adj(exptree):
    if exptree is None:
        return []
    stack = [exptree]
    ans = []
    ...
    while stack:
        ...
        if top.right:
            ...
            adj.append(top.right.value)
            #
        if top.left:
            stack.append(top.left)
            adj.append(top.left.value)
        adj_index.append(adj)
    return ans, adj_index

def prepare_data_infill(pairs_trained, pairs_tested_or_valed, trim_min_count, generate_nums, copy_nums, tree=False, use_bert=False, auto_transformer=False, bert_pretrain_path='../pretrain/bert-base-chinese'):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_or_val_pairs = ()
    ...
    for pair in pairs_trained:
        pair1 = copy.deepcopy(pair)
        tar_exp = pair[1]
        tar_exp_for_tree = pair1[1]
        try:
            exp_tree, index_tree = PreExpTree(tar_exp_for_tree)
        except:
            tar_exp_for_tree = ['1']
            tar_exp = ['1']
            exp_tree, index_tree = PreExpTree(tar_exp_for_tree)
        ...
        pre, adj_index = preorder_adj(index_tree)
        ...
        num_stack.reverse()
        input_cell = indexes_from_sentence(input_lang, pair[0])
        output_cell = indexes_from_sentence(output_lang, tar_exp, tree)

        posi = 0
        exp_num_pos = []
        exp_num_index = []
        for id in output_cell:
            token = output_lang.index2word[id]
            if token[0] == 'N':
                exp_num_pos.append(posi)
                exp_num_index.append(int(token[-1]))
            posi = posi + 1

        if use_bert:
            input_cell = bert_tokenizer.convert_tokens_to_ids(pair[0])
        train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
                            pair[2], pair[3], num_stack,  exp_num_pos, len(exp_num_pos), exp_num_index, adj_matrix))
...
def prepare_train_batch_infill(pairs_to_batch, batch_size, shuffle=True):
    pairs = copy.deepcopy(pairs_to_batch)
    if shuffle:
        random.shuffle(pairs)  # shuffle the pairs
    pos = 0
    ...
    for batch in batches:
        ...
        output_length = []
        tarexp_num_length = []
        for _, i, _, j, _, _, _, _, k, _, _ in batch:
            input_length.append(i)
            output_length.append(j)
            tarexp_num_length.append(k) #
        input_lengths.append(input_length)
        output_lengths.append(output_length)
        tarexp_num_lengths.append(tarexp_num_length) #
        input_len_max = max(input_length)
        output_len_max = max(output_length)
        exp_num_len_max = max(tarexp_num_length)
        input_batch = []
        output_batch = []
        num_batch = []
        num_stack_batch = []
        ...
        numlist_batch = []
        adj_tar_batch = []
        for i, li, j, lj, num, num_pos, num_stack, exp_pos, exp_numlen, exp_num_index, adj_tar in batch:
            num_batch.append(len(num))
            input_batch.append(pad_seq(i, li, input_len_max))
            output_batch.append(pad_seq(j, lj, output_len_max))
            num_stack_batch.append(num_stack)
            num_pos_batch.append(num_pos)
            num_size_batch.append(len(num_pos))
            exp_num_pos_batch.append(exp_pos + [0] * (exp_num_len_max - exp_numlen))
            exp_num_index_batch.append(exp_num_index + [0] * (exp_num_len_max - exp_numlen))
            numlist_batch.append(num)
            adj_tar_batch.append(pad_adj(adj_tar, output_len_max))
        ...
        num_pos_batches.append(num_pos_batch)
        num_size_batches.append(num_size_batch)
        exp_num_pos_batches.append(exp_num_pos_batch)
        exp_num_index_batches.append(exp_num_index_batch)
        numslist_batches.append(numlist_batch)
        adj_tar_batches.append(np.array(adj_tar_batch))

    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches, exp_num_pos_batches, tarexp_num_lengths, exp_num_index_batches, numslist_batches, adj_tar_batches
